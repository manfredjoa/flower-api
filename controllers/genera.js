...
export const getGeneraByFamily = async (req, res) => {
  try {
    ...
    const genera = await Genus.find({ familyName: upperCaseFamilyName });

    if (genera.length === 0) {
      return res
        .status(404)
        .json({ error: "No genera found by that family name." });
    } else {
      return res.json(genera);
    }
  } catch (error) {
