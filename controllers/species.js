...
export const getSpeciesByGenus = async (req, res) => {
  try {
    ...
    const species = await Species.find({ genusName: upperCaseGenusName });

    if (species.length === 0) {
      return res
        .status(404)
        .json({ error: "No species found by that genus name." });
    } else {
...
export const updateOneSpecies = async (req, res) => {
  const { speciesName } = req.params;
  const oneSpecies = await Species.findOneAndUpdate(
...
export const deleteOneSpecies = async (req, res) => {
  try {
    const { speciesName } = req.params;
    const deleted = await Species.findOneAndDelete({
      speciesName: speciesName,
    });
